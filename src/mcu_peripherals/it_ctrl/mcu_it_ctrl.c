...
void mcu_irq_init(void)
{
  ...
  //config_irq(EXTI3_IRQn,          IRQ_GPOUR_IMPORTANT,  1);                     // mains zerocrossing detection
  //config_irq(TIM2_IRQn,           IRQ_GPOUR_IMPORTANT,  2);                     // onewire timer
  //config_irq(EXTI2_IRQn,          IRQ_GPOUR_IMPORTANT,  3);                     // onewire sensor on EXTI line 2
  ...
  config_irq(EXTI15_10_IRQn,      IRQ_GROUP_USUAL, 5);                            // push-button encoder
}
