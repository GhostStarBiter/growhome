...
uint8_t esp8266_isStarted(void);        // Check if the module is started (AT)
uint8_t esp8266_restart(void);          // Restart module (AT+RST)
...
uint8_t esp8266_start(unsigned char protocol, char* ip, unsigned char port);

// Send data (AT+CIPSEND)
uint8_t esp8266_send(unsigned char*);
